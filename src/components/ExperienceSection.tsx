...
const ExperienceItem = ({ data }: { data: ExperienceInfo }) => {
  ...
  return (
    <div className="py-4 flex items-start gap-4 border-b last:border-b-0 border-white/10">
      <div
      ...
      <div className="flex-1">
        <div className="flex flex-col sm:flex-row gap-2 items-baseline justify-between">
          <div>
            ...
            <p className="md:text-lg text-[var(--p)]">{data.company}</p>
          </div>
          <p className="text-xs md:text-sm">{data.date}</p>
        </div>
        <div
          className="text-xs md:text-sm text-body markdown-box mt-1"
